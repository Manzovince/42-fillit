...
/*   Updated: 2019/05/01 22:50:17 by hulamy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		/* tests pour trouver la nouvelle equation
		...
		print_bits((mask & list->tetribit << (16 + i)), 32);
		print_bits((mask & list->tetribit << (16 + i)) << ((33 - j % 32) % 32), 32);
		print_bits(map[(j + list->width) / 32] | (mask & list->tetribit << (16 + i)) << ((33 - j % 32) % 32), 32);
//		*/

//		ancienne version avec ancienne equation
		/*
								print_map(map, size, size);																						// POUR DEBUG
								ft_putstr("map[j / 32]    : "); print_bits(map[j / 32], 32);													// POUR DEBUG
								ft_putstr("mask           : "); print_bits((mask >> i & list->tetribit << 16) >> (j - i - 1), 32);				// POUR DEBUG
		*/
//		map[j / 32] ^= (mask >> i & list->tetribit << 16) >> (j - i - 1);
		/*
								ft_putstr("map[j / 32]    : "); print_bits(map[j / 32], 32);													// POUR DEBUG
								ft_putstr("map[(j+s) / 32]: "); print_bits(map[(j + size) / 32], 32);											// POUR DEBUG
								ft_putstr("mask 2         : "); print_bits((mask >> i & list->tetribit << 16) << (list->width * (list->height - 1) + ((33 - (j % 32)) % 32)), 32);				// POUR DEBUG
		*/
//		map[(j + size) / 32] ^= (mask >> i & list->tetribit << 16) << (list->width * (list->height - 1) + ((33 - (j % 32)) % 32));
		/*
								ft_putstr("map[(j+s) / 32]: "); print_bits(map[(j + size) / 32], 32);											// POUR DEBUG
		*/
		j -= size;
